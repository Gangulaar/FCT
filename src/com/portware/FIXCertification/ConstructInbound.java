...
public class ConstructInbound 
{
	...
	public ConstructInbound()
	{
		...
		//	XMLParser.t.length();
			if(f.exists())
			{
				new FileHandler().truncateFile(f);
			}
			else
			{
	...
	public void buildInbound()
	{
		...
		for(Transaction t : XMLParser.t)
		{
			...
			if(typeOfOrder(QuoteRequest,OrderTransaction))
			{
		    	...
		    	Collections.addAll(TransactionSet, QuoteRequest ,QuoteResponse,OrderTransaction,ExecTransaction,InboundOrder);
	...
	{
		/*
		 * returns 0 if transaction type is Inbound
		 * returns 1 if transaction type is Discretionary  
		 * */
